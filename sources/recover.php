...
function PageMain() {
	...
	$TMPL['form_url'] = permalink($CONF['url'].'/index.php?a=recover');
	$TMPL['url'] = $CONF['url'];
	$rows .= $skin->make();
	...
	elseif(isset($_POST['username']) && !empty($_POST['username'])) {
		...
		if(strtolower($_POST['username']) == $username || strtolower($_POST['username']) == $email) {
			...
			sendMail($email, $LNG['recover_mail'], sprintf($LNG['recover_content'], $username, $salted, permalink($CONF['url'].'/index.php?a=recover&r=1'), permalink($CONF['url'].'/index.php?a=recover&r=1'), $CONF['url'], $settings['title']), $CONF['email']);
	...
	if(isset($_GET['r'])) {
		if(empty($_POST['n']) || empty($key) || (empty($_POST['u']) && empty($key))) {
			...
			$TMPL['form_url'] = permalink($CONF['url'].'/index.php?a=recover&r=1');
			$TMPL['url'] = $CONF['url'];
