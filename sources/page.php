...
function PageMain() {
	...
	foreach($page as $url => $value) {
		if($value[2]) {
			...
			$TMPL['links'] .= '<div class="sidebar-link'.$class.'"><a href="'.permalink($CONF['url'].'/index.php?a=page&b='.$url).'" rel="loadpage">'.skin::parse($value[0]).'</a></div>';
		}
	}
	$sidebar = $skin->make();
