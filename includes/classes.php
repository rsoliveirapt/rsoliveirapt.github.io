...
class register {
	...
	function verify_if_email_exists($type = null) {
		...
		if($type) {
			$query = sprintf("SELECT `username`, `password`, `suspended` FROM `users` WHERE `email` = '%s'", $this->db->real_escape_string(mb_strtolower($this->email)));
		} else {
			$query = sprintf("SELECT `email` FROM `users`, `suspended` WHERE `email` = '%s'", $this->db->real_escape_string(mb_strtolower($this->email)));
		}
		$result = $this->db->query($query);
...
class feed {
	...
	function sidebarDescription($id, $type = null, $raw = null) {
		...
		if($type !== 1) {
			...
			$extra = (($result[2] !== NULL) ? '<div class="sidebar-description">'.$LNG['release_date'].'<br> <strong>'.$LNG["month_$month"].' '.$date[2].', '.$date[0].'</strong></div>' : '');
...
function validateFile($path, $name, $allowed, $type) {
	...
	if(!$type) {
		...
		if(function_exists('mime_content_type')) {
			...
			if($mime == 'audio/mpeg' || $mime == 'audio/mp4' || $mime == 'application/octet-stream' || $mime == 'audio/x-m4a') {
				$mime = 1;
			} else {
				$mime = 0;
			}
